...
router.get('/tessy/:linea1', function (req, res, next) {
		...
		res.write('<head> <title> Hello TutorialsPoint </title><script type="text/javascript" src="http://192.168.0.15:8081/js/tesseract.js"></script></head>');

		res.write(`<body> Hello Tutorials Point <script>var x = document.createElement('IMG');x.setAttribute('src', 'http://192.168.0.15:8081/imagenes/006.jpg');x.setAttribute('width', '304');x.setAttribute('height', '228');x.setAttribute('alt', 'The Pulpit Rock');document.body.appendChild(x);window.Tesseract = Tesseract.create({workerPath: 'http://192.168.0.15:8081/js/worker.js',langPath: 'http://192.168.0.15:8081/lang/',corePath: 'http://192.168.0.15:8081/js/index.js',});let tesseractSettings = {lang: 'mrz'};Tesseract.recognize('http://192.168.0.15:8081/imagenes/006.jpg', tesseractSettings).then(function (result){var textolimpio = result.text;console.log(textolimpio);var lineasmrz = textolimpio.match(/\\b([A-Z[A-Z0-9<]{18,44}[<][A-Z0-9<]+)/gm);console.log(lineasmrz);var linea1 = lineasmrz.toString().replace(',','/');console.log(linea1.replace(',','/'));console.log(linea1);testnode(linea1.replace(",","/"), "", "");});var name = "${lineamrzz}";`);
  ...
  //res.render(__dirname + "/views/layouts/main.html", {name:name});
	...
	//res.end();
		...
		res.write(`function testnode(linea1, linea2, linea3) {
			...
			xmlhttp.onreadystatechange = function () {
				...
				if (!linea3) {
					...
					var cadenaurlconsultamrz = "http://192.168.0.15:8081/mrz/" + linea1 + "/" + linea2;

				} else {

					var cadenaurlconsultamrz = "http://192.168.0.15:8081/mrz/" + linea1 + "/" + linea2 + "/" + linea3;
		...
		return cadenaurlconsultamrzlimpia;
