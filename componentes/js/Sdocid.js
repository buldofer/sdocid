...
var escanearDoc = document.getElementById("escanearDoc");

var agregardoc = document.getElementById("agregardoc");

var crearxml = document.getElementById("crearxml");

var resetformulario = document.getElementById("resetformulario");

var labelbuscarimagen = document.getElementById("labelbuscarimagen");

var imgthumnil = document.getElementById("thumbnil");

var base64 = document.getElementById("base64");

var buscarimagen = document.getElementById("buscarimagen");

var tomadedatos = document.getElementById("tomadedatos");

var cuadromrz = document.getElementById("cuadromrz");

var confmrz = document.getElementById("confmrz");

var contenedorlienzofirma = document.getElementById("contenedorlienzofirma");
        ...
        .then(function(res) {
          if (res.status == 200) {
            ...
            if (res.data.fields.optional1) {
              ...
              personalNumber = res.data.fields.optional1;
            }

            if (res.data.fields.personalNumber) {
              $("#optional1").val(res.data.fields.personalNumber);

              personalNumber = res.data.fields.personalNumber;
            }
            ...
            documentType = res.data.fields.documentCode;

            issuer = res.data.fields.issuingState;

            lastName = res.data.fields.lastName;

            otherNames = res.data.fields.firstName;

            documentNumber = res.data.fields.documentNumber;

            nationality = res.data.fields.nationality;

            birthDate = res.data.fields.birthDate;

            sex = res.data.fields.sex;

            expiryDate = res.data.fields.expirationDate;

            $("#botoninvisible").click();

            $("#editardatos").css("display", "block");

            $("#guardarfirma").css("display", "block");

            $("#textofirma").css("display", "block");
...
// muy importante está función como está 
    ...
    canvasimagendocu.getContext("2d").drawImage(imagendocumento, 0, 0);
	 ...
	 $.ajax({
      type: "POST",
      url: "/sdocid/us-central1/crearimgdoc",
      data: {
	  ...
	  $.ajax({
      url: "/sdocid/us-central1/crearimgout"
      }
     )
  ...
  async function guardarfirmadoc() {
    $.ajax({
      type: "POST",
      url: "/sdocid/us-central1/guardarimagen",
      data: {
 ...
 async function getmrz() 
 {
       const response = await axios.get('/sdocid/us-central1/tess', {
          responseType: "text"
        }).then(function(res) {
           var myObj = JSON.stringify(res);

          var sss = JSON.parse(myObj);

            console.log(sss.data);

            cadenaurlconsultamrz = sss.data;
 })
  ...
  $("#buttondatosreserva").on("click",function () {
  ...
  // $("#listaviviendasarevisar").css("height", "50%");
    ...
    setTimeout(function () {
     ...
     $("#listaviviendasarevisar").css("height", (lssro * listaviviendasarevisar.options.length+40));
  ...
  $("#listaviviendasarevisar").on("click",function () {
...
var bt1 = document.getElementById('butto');

bt1.style.color = o.color;

// window.location.replace(o.reloca);
  ...
  async function combolistareservas() {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      dbdd.inquilinos.each(function (inquilinos) {
       ...
       //  $("#listaviviendasarevisar").css("height", "7%");
...
const cleanText = string => {
  ...
  for (var i = 0; i < specialChars.length; i++) {
    string = string
      .toString()
      .replace(new RegExp("\\" + specialChars[i], "gm"), "");
  }

  string = string.toString().replace(/([\r\\])+/gm, "");
  return string;
};
...
const but = () => {
  ...
  const bt1 = document.querySelector("body");

 
  return bt1.innerHTML;
